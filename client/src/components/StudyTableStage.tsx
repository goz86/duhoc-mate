...
export default function StudyTableStage({
  ...
  const expandedVideo = useMemo(() => {
    ...
    return videoParticipants.find(item => item.member.id === expandedVideoMemberId) || null
  }, [expandedVideoMemberId, videoParticipants]) as (typeof videoParticipants)[number]

  useEffect(() => {
    if (expandedVideoMemberId && !videoParticipants.some(item => item.member.id === expandedVideoMemberId)) {
      setExpandedVideoMemberId(null)
    }
  }, [expandedVideoMemberId, videoParticipants])
  ...
  return (
    <div className="flex flex-1 flex-col gap-4">
      ...
      <div className="grid min-w-0 flex-1 gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
        <section className="relative min-w-0 overflow-visible rounded-[28px] border border-brand-terracotta-light/20 bg-[#FDF8F0] p-3 shadow-[0_24px_70px_rgba(76,55,49,0.08)] sm:min-h-[430px] sm:p-5 xl:min-h-[460px]">
          ...
          <div className="relative z-10 flex min-h-[330px] min-w-0 flex-col sm:min-h-[390px] xl:min-h-[420px]">
            ...
            {showVideoWall && activeVideoParticipant && (
              <div className="mb-4 min-w-0 overflow-hidden rounded-[24px] border border-white/70 bg-white/78 p-2.5 shadow-[0_16px_40px_rgba(76,55,49,0.10)] backdrop-blur">
                ...
                {videoLayoutMode === 'grid' ? (
                  ...
                  <div className="grid min-w-0 gap-2 lg:grid-cols-[minmax(0,1fr)_minmax(132px,160px)]">
                    <button
                      type="button"
                      onClick={() => {
                        setPinnedVideoMemberId(activeVideoParticipant.member.id)
                        setVideoLayoutMode('pinned')
                      }}
                    ...
                    <div className="grid min-w-0 grid-cols-2 gap-2 overflow-visible sm:max-h-[260px] sm:overflow-y-auto lg:grid-cols-1">
                      {videoParticipants.map(item => (
                        <button
                          key={item.member.id}
                          type="button"
            ...
            <div className="relative min-h-0 flex-1">
              ...
              <div className={`relative grid max-h-[min(62vh,620px)] w-full ${gridClass} content-start gap-2 overflow-y-auto overflow-x-visible pr-1 sm:gap-3 xl:gap-4`}>
                ...
                {filteredSeats.map((member, index) => {
                  ...
                  const isCardVideoExpanded = showCardVideo && expandedVideoMemberId === member.id && !isMicro
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        ...
                        {isCardVideoExpanded && videoStream && (
                          <div className="mb-3 overflow-hidden rounded-[20px] border border-white/80 bg-slate-950 shadow-[0_14px_30px_rgba(15,23,42,0.16)]">
                            <button
                              type="button"
                              onClick={() => setExpandedVideoMemberId(null)}
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          {showCardVideo && !isMicro && videoStream && !isCardVideoExpanded && (
                            <button
                              type="button"
